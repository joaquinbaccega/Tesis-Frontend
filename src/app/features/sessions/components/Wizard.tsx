...
import { saveDraftStep, submitSession } from '../services/session.api';
import { mapFormToPayload } from '../utils/submitMapper';

// Tus steps (ya los tenés creados)
...
import { fetchDrafts } from '../services/session.api';
...
const Wizard: React.FC = () => {
  ...
  const [sessionId] = useState<string>(() => typeof window !== 'undefined' ? crypto.randomUUID() : 'session');

  const onSaveDraft = useCallback(async () => {
    const data = methods.getValues();
    // Guardar solo lo necesario (podés filtrar drafts aquí si preferís)
  ...
  useEffect(() => {
    (async () => {
      try {
        const drafts = await fetchDrafts(sessionId);
        if (!Array.isArray(drafts)) return;
        const merged = drafts
          .sort((a, b) => a.step - b.step)
          .reduce((acc, d) => deepMerge(acc, d.data), { ...defaultValues });
        methods.reset(merged); // rehídrata el form
      } catch (e) {
        // silencioso: si no hay borradores, seguimos con defaults
      ...
      <Box>
        ...
        <Box display="flex" gap={2} justifyContent="space-between">
          ...
          <Box display="flex" gap={2}>
            <Button variant="text" onClick={onSaveDraft}>Guardar borrador</Button>
            {activeStep < steps.length - 1
              ? <Button variant="contained" onClick={onNext}>Siguiente</Button>
              : <Button variant="contained" onClick={onSubmit}>Finalizar</Button>}
          </Box>
        </Box>
      </Box>
    </FormProvider>
